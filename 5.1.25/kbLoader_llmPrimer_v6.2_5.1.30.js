...
;(function(){
        ...
        if (initPanel) {
          ...
          setTimeout(() => {
            ...
            if (categorySelect && kbSelect) {
              ...
              // Add explicit delayed call to create the link for the default selection
              setTimeout(() => {
                debugLog('Creating initial promptInject link...');
                window.updatePromptInjectLink();
              }, 500);  // Half-second delay to ensure everything is ready
            }
          }, 100);
        }
      }
    ...
    window.updatePromptInjectLink = function() {
      try {
        debugLog('updatePromptInjectLink called');
        ...
        debugLog(`Found knowledgeBaseSelect with value: ${kbSelect.value}`);
        ...
        debugLog(`Found selectedKb: ${selectedKb.id}`);
        ...
        debugLog(`Found ${existingLinks.length} existing links`);
        
        if (existingLinks.length > 1) {
          debugLog('Removing extra links');
        ...
        // Use the actual promptInject value as the link text
        newLink.textContent = selectedKb.promptInject;
    ...
    function populateKnowledgeBaseDropdown(categoryId, selectElement) {
      ...
      if (kbsForCategory.length > 0) {
        ...
        // Update the promptInject link inside populateKnowledgeBaseDropdown
        // Note: This might run too early, which is why we also call it with a delay later
        window.updatePromptInjectLink();
      } else {
        selectElement.value = '';
        selectElement.disabled = false;
