...
const kbResearchModule = () => {
    ...
    const utils = {
        ...
        // Enhanced preserve formatting to maintain rich text structure
        preserveFormatting: function(content) {
            if (!content) return '';
            
            // If content is already a string, return as is to preserve any HTML/markdown
            if (typeof content === 'string') {
                return content;
            }
            
            // For arrays, preserve formatting of each item
            if (Array.isArray(content)) {
                return content.map(item => this.preserveFormatting(item)).join('\n');
            }
            
            // For objects, convert to formatted string
        ...
        // Convert plain text to markdown with preserved formatting
        convertToMarkdown: function(text, level = 0) {
            if (!text) return '';
            
            // If already contains markdown or HTML, preserve as is
            ...
            return lines.map(line => {
                ...
                // Check if it looks like a heading (ends with : or .)
                if (line.endsWith(':') && line.length < 50) {
                    return `${'#'.repeat(level + 2)} ${line}`;
                }
                
                // Check if it looks like a list item
                if (line.startsWith('-') || line.startsWith('•')) {
                    return line; // Already formatted as list
                }
    ...
    const kbResearch = {
         ...
         */
        initialize: function() {
            utils.log("Initializing Knowledge Base...");
            ...
            knowledgeBaseConfig.forEach(feature => {
                this._loadFeature(feature, () => {
                    ...
                    if (loadedCount === totalFeatures) {
                        utils.log("Knowledge Base initialized successfully");
                    }
                });
            });
        },
        ...
        generateFeaturePrompt: function(data) {
            // Structure the prompt with all available sections and maintain rich text formatting
            let prompt = `# ${utils.getProperty(data, 'title') || utils.getProperty(data, 'name')} Knowledge Base\n\n`;
            
            // Add formatted knowledge base section with preserved rich text
            prompt += this.formatGenericKnowledgeBase(data);
            
            // Add LLM instructions - preserve any HTML/markdown formatting
            ...
            // Add LLM rules if present - preserve any HTML/markdown formatting
            ...
            // Add knowledge base if present - preserve any HTML/markdown formatting
            ...
            // Add user interaction instructions - preserve any HTML/markdown formatting
         ...
         * @param {Object} data - The feature data
         * @returns {string} - Formatted knowledge base section with preserved rich text
         */
        formatGenericKnowledgeBase: function(data) {
            let result = '';
            
            // Iterate through data properties and format them, preserving rich text
            for (const [key, value] of Object.entries(data)) {
                ...
                if (typeof value === 'object' && value !== null) {
                    ...
                    if (Array.isArray(value)) {
                        // Format array items - preserve rich text formatting for each item
                        ...
                        // Format nested objects - preserve formatting
        ...
        /**
         * Format object properties for display with preserved rich text
         * @param {Object} obj - Object to format
         * @param {boolean} preserveFormat - Whether to preserve HTML/markdown formatting
         * @returns {string} - Formatted string with preserved rich text
         */
